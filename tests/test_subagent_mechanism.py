...
from clear_agent.tools.tool_filter import ReadOnlyFilter, FullAccessFilter
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        if self.tool_registry and "使用工具" in input_text:
            ...
            if tools:
                tool_name = tools[0]
                self.add_message(
                    Message(role="assistant", content=f"Action: {tool_name}[test]")
                )
...
class TestAgentRunAsSubagent:
    ...
    def test_basic_subagent_execution(self):
        ...
        # 执行子代理任务
        result = agent.run_as_subagent(task="测试任务", return_summary=True)
    ...
    def test_tool_filter_readonly(self):
        ...
        result = agent.run_as_subagent(task="使用工具完成任务", tool_filter=tool_filter)
    ...
    def test_return_full_result(self):
        ...
        result = agent.run_as_subagent(task="测试任务", return_summary=False)
    ...
    def test_max_steps_override(self):
        ...
        # 使用覆盖值执行
        result = agent.run_as_subagent(task="测试任务", max_steps_override=5)
