...
from clear_agent.core.session_store import SessionStore
from clear_agent.core.message import Message
from clear_agent.core.agent import Agent
from clear_agent.core.llm import ClearAgentLLM
from clear_agent.core.config import Config
from clear_agent.agents.simple_agent import SimpleAgent
...
class TestSessionStore:
    ...
    def test_save_and_load_session(self):
        ...
        history = [Message("Hello", "user"), Message("Hi there!", "assistant")]
...
class TestAgentSessionPersistence:
    ...
    def test_agent_save_session(self):
        ...
        # 创建 Agent
        llm = ClearAgentLLM()
        agent = SimpleAgent(name="test_agent", llm=llm, config=self.config)
    ...
    def test_agent_load_session(self):
        ...
        # 创建并保存会话
        llm = ClearAgentLLM()
        agent1 = SimpleAgent(name="agent1", llm=llm, config=self.config)
        ...
        # 创建新 Agent 并加载会话
        agent2 = SimpleAgent(name="agent2", llm=llm, config=self.config)
    ...
    def test_session_disabled(self):
        ...
        llm = ClearAgentLLM()
        agent = SimpleAgent(name="test_agent", llm=llm, config=config)
    ...
    def test_compute_tool_schema_hash(self):
        ...
        agent = SimpleAgent(
            name="test_agent", llm=llm, config=self.config, tool_registry=registry
        )
