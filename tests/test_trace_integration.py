...
class TestTraceIntegration:
    ...
    def test_react_agent_with_trace(self):
        ...
        # 配置
        config = Config(
            trace_enabled=True, trace_dir=self.temp_dir, trace_sanitize=True
        )
        ...
        # 创建 Agent
        agent = ReActAgent(
            name="TestAgent", llm=llm, tool_registry=tool_registry, config=config
        )
    ...
    def test_trace_files_generated(self):
        ...
        if agent.trace_logger:
            ...
            agent.trace_logger.log_event(
                "tool_call", {"tool_name": "Calculator"}, step=1
            )
