...
import pytest
import time
from clear_agent.tools.circuit_breaker import CircuitBreaker
from clear_agent.tools.response import ToolResponse, ToolStatus
from clear_agent.tools.errors import ToolErrorCode
from clear_agent.tools.registry import ToolRegistry
from clear_agent.tools.base import Tool
...
class TestCircuitBreaker:
    ...
    def test_failure_threshold(self):
        ...
        for i in range(3):
            response = ToolResponse.error(
                code=ToolErrorCode.EXECUTION_ERROR, message="Test error"
            )
    ...
    def test_get_all_status(self):
        ...
        # 工具 A：1 次失败
        response = ToolResponse.error(
            code=ToolErrorCode.EXECUTION_ERROR, message="Error"
        )
...
class DummyTool(Tool):
    ...
    def __init__(self, should_fail: bool = False):
        super().__init__(name="dummy_tool", description="A dummy tool for testing")
    ...
    def run(self, parameters: dict) -> ToolResponse:
        ...
        if self.should_fail:
            return ToolResponse.error(
                code=ToolErrorCode.EXECUTION_ERROR, message="Dummy tool failed"
            )
        return ToolResponse.success(text="Success")

    def get_parameters(self):
        """返回工具参数定义"""
        from clear_agent.tools.base import ToolParameter
