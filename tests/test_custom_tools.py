...
import pytest
from clear_agent.tools import Tool, ToolParameter, ToolResponse, ToolRegistry
...
class SimpleTestTool(Tool):
    ...
    def __init__(self):
        super().__init__(name="simple_test", description="简单测试工具")

    def run(self, parameters):
        text = parameters.get("text", "")
        if not text:
            return ToolResponse.error(
                code=ToolErrorCode.INVALID_PARAM, message="参数 'text' 不能为空"
            )
    ...
    def get_parameters(self):
        return [
            ToolParameter(
                name="text", type="string", description="要处理的文本", required=True
            )
        ]
...
class TestFunctionTools:
    ...
    def test_register_function_with_custom_name(self):
        ...
        registry.register_function(
            my_function, name="custom_name", description="自定义描述"
        )
...
class TestExpandableTools:
    ...
    def test_expandable_tool_registration(self):
        """测试可展开工具的注册"""
        from clear_agent.tools import tool_action

        class MultiTool(Tool):
            def __init__(self):
                super().__init__(
                    name="multi", description="多功能工具", expandable=True
                )
            ...
            def run(self, parameters):
                return ToolResponse.error(
                    code="NOT_IMPLEMENTED", message="请使用子工具"
                )
