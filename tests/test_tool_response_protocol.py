...
class TestToolResponse:
    ...
    def test_success_response(self):
        ...
        resp = ToolResponse.success(
            text="计算结果: 42", data={"result": 42, "expression": "6*7"}
        )
    ...
    def test_partial_response(self):
        """测试部分成功响应"""
        resp = ToolResponse.partial(text="结果已截断", data={"truncated": True})
    ...
    def test_error_response(self):
        """测试错误响应"""
        resp = ToolResponse.error(code=ToolErrorCode.NOT_FOUND, message="文件不存在")
