...
class TestMessage:
    ...
    def test_to_dict_with_metadata(self):
        """测试 to_dict 包含完整信息"""
        msg = Message("测试内容", "user", metadata={"key": "value"})
        data = msg.to_dict()
...
class TestObservationTruncator:
    ...
    def test_truncation_direction_tail(self):
        ...
        truncator = ObservationTruncator(
            max_lines=5, truncate_direction="tail", output_dir=self.test_output_dir
        )
    ...
    def test_agent_real_conversation_with_compression(self):
        ...
        for i, question in enumerate(questions):
            ...
            try:
                ...
                print(
                    f"当前历史: {len(history)} 条消息, {rounds} 轮对话, 包含摘要: {has_summary}"
                )
