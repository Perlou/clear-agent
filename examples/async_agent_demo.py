...
import asyncio
from clear_agent.core.llm import HelloAgentsLLM
from clear_agent.core.config import Config
from clear_agent.core.lifecycle import AgentEvent, EventType
...
class CalculatorTool(Tool):
    ...
    def run(self, parameters: dict) -> ToolResponse:
        ...
        try:
            ...
            return ToolResponse.error(code="CALC_ERROR", message=f"计算失败: {str(e)}")
...
async def main():
    ...
    # 4. 创建 Agent
    agent = ReActAgent(
        name="AsyncAgent", llm=llm, tool_registry=registry, config=config, max_steps=5
    )
