...
```python
from clear_agent import ReActAgent, ClearAgentLLM, Config

# 启用子代理机制
config = Config(subagent_enabled=True)
agent = ReActAgent("main", ClearAgentLLM(), config=config)
...
from clear_agent.tools.tool_filter import ReadOnlyFilter
...
```python
from clear_agent.tools.tool_filter import (
...
**create_agent() - 统一创建接口：**
```python
from clear_agent.agents.factory import create_agent
...
```python
from clear_agent.agents.factory import default_subagent_factory
...
```python
from clear_agent.agents.factory import create_agent, default_subagent_factory
from clear_agent.tools.builtin.task_tool import TaskTool

# 主模型（强大但昂贵）
main_llm = ClearAgentLLM(provider="openai", model="gpt-4")

# 轻量模型（快速且便宜）
light_llm = ClearAgentLLM(provider="deepseek", model="deepseek-chat")
