...
**Function Calling 架构**是 ClearAgent 框架的核心重构，将 LLM 基类和所有 Agent 类型统一为 Function Calling 模式，解析成功率从 85% 提升到 99%+。
...
```python
from clear_agent import ReActAgent, ClearAgentLLM, ToolRegistry
from clear_agent.tools.builtin import ReadTool, SearchTool
...
agent = ReActAgent("assistant", ClearAgentLLM(), tool_registry=registry)
...
```python
from clear_agent.llm import ClearAgentLLM
from clear_agent.tools.builtin import ReadTool
...
```python
from clear_agent import ReActAgent, ClearAgentLLM, ToolRegistry
from clear_agent.tools.builtin import ReadTool, WriteTool
...
```python
from clear_agent import ReflectionAgent, ClearAgentLLM

agent = ReflectionAgent("thinker", ClearAgentLLM(), tool_registry=registry)
...
```python
from clear_agent import PlanSolveAgent, ClearAgentLLM

agent = PlanSolveAgent("planner", ClearAgentLLM(), tool_registry=registry)
...
agent = SimpleAgent("assistant", ClearAgentLLM(), tool_registry=registry)
...
```python
from clear_agent.core.lifecycle import LifecycleHook
...
A: 使用 TraceLogger：
```python
from clear_agent.core.observability import TraceLogger
