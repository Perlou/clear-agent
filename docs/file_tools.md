...
文件工具已内置在 ClearAgent 框架中，无需额外安装。

### 基本使用

```python
from clear_agent import ToolRegistry, ReActAgent, ClearAgentLLM
from clear_agent.tools.builtin import ReadTool, WriteTool, EditTool
...
```python
from clear_agent.tools.builtin import MultiEditTool
