...
Skills 是 ClearAgent 的知识外化系统，允许你将领域知识写成独立的 Markdown 文件，Agent 会在需要时自动加载。
...
from clear_agent.tools import ToolRegistry

# 创建 Agent（框架会自动检测 skills/ 目录）
agent = ReActAgent(
    name="assistant",
    llm=ClearAgentLLM(provider="openai", model="gpt-4"),
