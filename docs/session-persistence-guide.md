...
```python
from clear_agent import SimpleAgent, ClearAgentLLM, Config

# 创建 Agent（默认启用会话持久化）
config = Config(session_enabled=True)
agent = SimpleAgent("assistant", ClearAgentLLM(), config=config)
...
```python
from clear_agent import ReActAgent, ToolRegistry
