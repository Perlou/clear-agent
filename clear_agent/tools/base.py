...
from typing import Dict, Any, List, Optional, Callable, cast, get_type_hints
...
def tool_action(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        action = cast(Any, func)
        action._is_tool_action = True
        action._tool_name = name
        action._tool_description = description
        return func
    ...
    def get_expanded_tools(self) -> Optional[List["Tool"]]:
        ...
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            if hasattr(method, "_is_tool_action"):
                action = cast(Any, method)
                tool = AutoGeneratedTool(
                    parent=self,
                    method=method,
                    name=action._tool_name,
                    description=action._tool_description,
                )
                tools.append(tool)
...
class AutoGeneratedTool(Tool):
    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        try:
            ...
            return ToolResponse.error(
                code=ToolErrorCode.EXECUTION_ERROR, message=f"方法执行失败: {str(e)}"
            )
